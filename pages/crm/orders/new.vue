      ...
      <form @submit.prevent="saveOrder()">
        ...
        <div class="mb-3">
          ...
          <div v-if="selected" id="emailHelp" class="form-text">
            Выбрана турбина:
            <b>{{ turbine.Turbo_Maker }} {{ turbine.Turbo_Model }}</b>
          </div>
        </div>
        <div class="mb-3">
          <label for="exampleInputEmail12" class="form-label"
            >Предворительная стоимость</label
          >
          <input
            type="text"
            v-model="prePrice"
...
export default {
  ...
  methods: {
    getTurbine() {
      this.$axios
        .$get("https://turbo69.ru/turbine.json")
        .then((response) => (this.turbineList = response));
    },
    selectTurbine(item) {
      this.turbine = item;
      this.selected = true;
    ...
    async saveOrder() {
      if (this.turbine.OE_No == undefined) {
        this.turbine = {
          ...this.turbine,
          OE_No: this.query,
        };
