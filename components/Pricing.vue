      ...
      <div v-for="(item, key) of pricing" :key="key" class="col-md-3 mb-5 mx-auto">
        <div class="card h-100 shadow">
          <div class="card-body">
            ...
            <div class="card-text">
                <ul class="fa-ul">
                  <li v-for="(service, key) in item.services" :key="key"><span class="fa-li"><i class="fas fa-check"></i></span>{{service}}</li>
                </ul>
            </div>
          </div>
          <div class="card-footer">
            <button v-if="$device.isDesktop" @click="$bvModal.show('bv-modal-form')" class="btn btn-primary">Отправить заявку &raquo;</button>
            <nuxt-link v-else to="/incom" class="btn btn-primary">Отправить заявку &raquo;</nuxt-link>
          </div>
        </div>
      </div>
    ...
    export default {
      name: "Pricing",
        data() {
            return {
                pricing: [
                    {title: "Ремонт турбины", price: 8500, services: ['Проверка состояния и зазоров ГМУ', 'Проверка герметичности ГМУ', 'Замена необходимых зачастей', 'Балансировка вала', 'Балансировка турбины']},
                    {title: "Профилактика турбины", price: 3500, services: ['Проверка герметичности ГМУ', 'Балансировка турбины']},
                    {title: "Ремонт электронного актуатора", price: 4500, services: ['Замена необходимых запчастей', 'Проверка крайних положений и плавности хода актуатора']},
                ],
                form: {
                  name: '',
                  phone: ''
